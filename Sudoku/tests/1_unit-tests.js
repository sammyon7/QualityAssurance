...
suite('UnitTests', () => {
  ...
  suite('Function isValidPuzzleGridEntry()', () => {
    test('Valid "1-9" characters', (done) => {
      const input = ['1', '2', '3', '4', '5', '6', '7', '8', '9'];
      
      input.forEach( elem => assert.isTrue(Solver.isValidPuzzleGridEntry(elem)))
      done();
    });
    ...
    test('Invalid characters (anything other than "1-9") are not accepted', (done) => {
      ...
      input.forEach( elem => assert.isFalse(Solver.isValidPuzzleGridEntry(elem)))
      done();
    });
  });
  
  suite('Function stringToArray()', () => {
    test('Parses a valid puzzle string into an object', done => {
      const input = '..9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..';
     
      const result = Solver.stringToArray(input)
